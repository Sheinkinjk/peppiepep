...
function tileFor(category: string, kind: SearchEntry["kind"]): Tile {
  ...
  if (c.includes("weight") || c === "health") return { Icon: Activity, fg: "#007a95", bg: "#e4f2f5" };
  if (c.includes("hair")) return { Icon: Scissors, fg: "#007a95", bg: "#e4f2f5" };
...
function highlight(title: string, q: string) {
  ...
  return (
    <>
      {title.slice(0, i)}
      <mark className="bg-transparent font-bold text-[#14120f]">{title.slice(i, i + t.length)}</mark>
  ...
  return (
    <div ref={wrapRef} className={`relative ${big ? "w-full max-w-xl" : "w-full max-w-xs"}`}>
      <div className={`nw-search ${big ? "" : "!py-2 !px-4"}`}>
        <Search className={`shrink-0 text-[#56504a] ${big ? "h-5 w-5" : "h-4 w-4"}`} />
        <input
          value={q}
      ...
      {open && (
        <div className={`absolute top-[calc(100%+8px)] z-[60] overflow-hidden rounded-2xl border border-[#ded8cd] bg-white shadow-[0_24px_60px_-24px_rgba(20,18,15,0.4)] ${big ? "left-0 right-0" : "right-0 w-[22rem]"}`}>
          {/* ── Results ── */}
            ...
            <ul className="max-h-[24rem] overflow-y-auto py-1.5">
              {ordered.map((r, i) => {
                ...
                return (
                  <li key={r.href}>
                    {showHeader && (
                      <p className="px-4 pb-1 pt-2.5 text-[11px] font-bold uppercase tracking-[0.14em] text-[#56504a]">
                        {KIND_LABEL[r.kind]}
                      </p>
                    )}
                    <button
                      ...
                      onClick={() => go(r.href)}
                      className={`flex w-full items-center gap-3 px-3 py-2 text-left transition-colors ${i === active ? "bg-[#f7f4ee]" : ""}`}
                    >
                      ...
                      <span className="min-w-0 flex-1">
                        <span className="block truncate text-sm font-semibold text-[#14120f]">{highlight(r.title, q)}</span>
                        <span className="text-xs text-[#56504a]">{r.category}</span>
                      </span>
                      <ArrowRight className={`h-4 w-4 shrink-0 transition-opacity ${i === active ? "text-[#007a95] opacity-100" : "opacity-0"}`} />
                    </button>
                  </li>
                );
              })}
          ...
          {hasQuery && ordered.length === 0 && (
            <div className="px-4 pb-1 pt-4">
              <p className="text-sm text-[#56504a]">
                No matches for <span className="font-semibold text-[#14120f]">&ldquo;{q.trim()}&rdquo;</span>. Browse instead:
              </p>
                ...
                {BROWSE.map((b) => {
                  ...
                  return (
                    <button key={b.href} onClick={() => go(b.href)}
                      className="group flex items-center gap-2.5 rounded-xl px-2 py-2 text-left transition-colors hover:bg-[#f7f4ee]">
                      ...
                      <span className="truncate text-[13px] font-semibold text-[#14120f] group-hover:text-[#007a95]">{b.label}</span>
                    </button>
                  );
                })}
              </div>
          ...
          {!hasQuery && (
            <div className="px-4 py-3.5">
              <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-[#56504a]">Browse</p>
              <div className="mt-2 grid grid-cols-2 gap-1">
                {BROWSE.map((b) => {
                  const t = tileFor(b.cat, "Category");
                  return (
          ...
          <div className="flex items-center justify-between border-t border-[#f1ede4] bg-[#fafcfb] px-4 py-2">
            <span className="hidden items-center gap-3 text-[11px] text-[#56504a] sm:flex">
              <span className="inline-flex items-center gap-1"><kbd className="rounded border border-[#ded8cd] bg-white px-1 font-sans text-[10px]">↑</kbd><kbd className="rounded border border-[#ded8cd] bg-white px-1 font-sans text-[10px]">↓</kbd> navigate</span>
              <span className="inline-flex items-center gap-1"><kbd className="rounded border border-[#ded8cd] bg-white px-1 font-sans text-[10px]"><CornerDownLeft className="h-2.5 w-2.5" /></kbd> open</span>
            </span>
            <button onClick={() => go(hasQuery ? `/search?q=${encodeURIComponent(q.trim())}` : "/guides")} className="ml-auto inline-flex items-center gap-1 text-xs font-semibold text-[#007a95] hover:text-[#003647]">
