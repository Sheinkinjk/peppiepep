...
export default function OffersTable({
  ...
  return (
    ...
    <div className="rounded-2xl border border-[#ded8cd] sm:overflow-x-auto">
      <table className="w-full border-collapse text-left text-sm">
        ...
        <tbody className="text-[#56504a]">
          {deals.map((d) => (
            <tr key={d.brand} className="block border-t border-[#f1ede4] px-4 py-4 first:border-t-0 sm:table-row sm:p-0 sm:first:border-t">
              <th scope="row" className="block text-[15px] font-semibold text-[#14120f] sm:table-cell sm:px-4 sm:py-3 sm:text-sm">
                {d.href ? (
                  <Link href={d.href} className="hover:text-[#007a95] hover:underline">{d.brand}</Link>
                ) : (
                  d.brand
                )}
              </th>
              <td className="mt-1 block sm:mt-0 sm:table-cell sm:px-4 sm:py-3">{d.offer}</td>
              <td className="hidden px-4 py-3 font-semibold tabular-nums text-[#14120f] sm:table-cell">{saving(d.offer)}</td>
              ...
              <td className="mr-3 mt-3 inline-block sm:mr-0 sm:mt-0 sm:table-cell sm:px-4 sm:py-3"><span className="inline-flex items-center gap-1 font-medium text-[#007a95]">Active ✓</span></td>
              {/* No fallback to the global sweep date. The footnote below promises
                  ...
                  manual visit. */}
              <td className="mt-3 inline-block whitespace-nowrap tabular-nums text-xs text-[#56504a] sm:mt-0 sm:table-cell sm:px-4 sm:py-3 sm:text-sm sm:text-[#56504a]">
                <span className="sm:hidden">Checked </span>
                {d.verified ? formatVerifiedFull(d.verified) : <span className="text-[#56504a]">Not recorded</span>}
              </td>
            </tr>
          ))}
        </tbody>
