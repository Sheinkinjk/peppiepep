...
type ReferralStatsClientProps = {
  initialCode: string;
  token: string | null;
};

export default function ReferralStatsClient({
  initialCode,
  token,
}: ReferralStatsClientProps) {
  ...
  useEffect(() => {
    const fetchData = async (referralCode: string) => {
      if (!token) {
        setError(
          "Secure token missing. Refresh the page or regenerate the ambassador link.",
        );
        setLoading(false);
        return;
      }
  ...
  const newUserReward =
    customer?.business?.new_user_reward_text || offerText;
  const clientReward =
    customer?.business?.client_reward_text ||
    `$${rewardAmount} credit for every conversion`;
  const rewardTerms = customer?.business?.reward_terms || null;
  const personalLink =
    customer?.referral_code ? `${siteUrl}/r/${customer.referral_code}` : null;

  const total = referrals.length;
  const earnedReferrals = referrals.filter((r) => r.status === "completed");
  const earned = earnedReferrals.length;
  ...
  const shareViaEmail = () => {
    if (personalLink) {
      const subject = encodeURIComponent(
        `Get ${newUserReward} at ${
          customer?.business?.name || "our business"
        }!`,
      );
      const body = encodeURIComponent(
        `Hey! I wanted to share this exclusive offer with you.\n\nGet ${newUserReward} when you use my referral link:\n${personalLink}\n\nI’ll earn ${clientReward} once you join. ${
          rewardTerms ? `\n\nTerms: ${rewardTerms}` : ""
        }`,
      );
  ...
  const shareViaSMS = () => {
    if (personalLink) {
      const message = encodeURIComponent(
        `Get ${newUserReward}! Use my referral link: ${personalLink}`,
      );
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-slate-900 via-purple-900 to-black flex items-center justify-center px-4 text-center">
        <div className="rounded-2xl border border-red-400/40 bg-red-500/10 p-8 text-white max-w-lg space-y-4 shadow-2xl">
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <p className="mt-4 text-xl text-slate-300/90 sm:text-2xl max-w-3xl">
              Turn your network into cash. Earn <span className="font-black text-transparent bg-gradient-to-r from-amber-400 to-orange-400 bg-clip-text">{clientReward}</span> for every friend you bring, and they get <span className="font-black text-transparent bg-gradient-to-r from-emerald-400 to-teal-400 bg-clip-text">{newUserReward}</span> too.
            </p>
          </div>
        </div>
                ...
                <div className="relative">
                  ...
                  <div className="mt-4 rounded-xl border border-emerald-300/40 bg-emerald-900/20 p-3 text-sm">
                    <p className="font-bold text-emerald-300 flex items-center gap-2">
                      ...
                      {clientReward} per successful referral
                    </p>
                  </div>
                </div>
              </Card>
                ...
                {referrals.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {referrals.map((referral) => {
                      const isEarned = referral.status === "completed";
                      return (
                        <div
                          key={referral.id}
                      ...
                      <div className="flex gap-4" key={step}>
                        ...
                        <div>
                          ...
                          <p className="text-sm text-slate-200/80">
                            {step === 1
                              ? "Copy and share via email, SMS, or social media"
                              : step === 2
                              ? `They get ${rewardAmount ? `$${rewardAmount}` : "a reward"} credit automatically applied`
                              : `Earn ${rewardAmount ? `$${rewardAmount}` : "VIP rewards"} when they complete their purchase`}
                          </p>
                        </div>
                      </div>
                    ))}
