...
export default async function ReferralPage({ params }: ReferralPageProps) {
  ...
  async function submitReferral(formData: FormData) {
    ...
    await supabaseAny.from("referrals").insert([
      {
        business_id: customerAny.business_id,
        ambassador_id: customerAny.id,
        referred_name: formData.get("name"),
        referred_phone: formData.get("phone"),
        status: "pending",
      },
    ]);
    revalidatePath("/");
  ...
  return (
    <div className="aurora flex min-h-screen items-center justify-center bg-gradient-to-b from-pink-50 via-white to-white p-4">
      <Card className="relative w-full max-w-md overflow-hidden rounded-2xl border border-white/70 bg-white/90 p-8 shadow-2xl backdrop-blur">
        ...
        <p className="mb-6 text-lg leading-relaxed text-slate-700">
          {customerAny.name} wants you to get{" "}
          <span className="font-semibold text-slate-900">{business.offer_text}</span> at{" "}
