      ...
      <main id="main-content">
        <section className="mx-auto max-w-6xl px-5 pt-12 sm:px-8 sm:pt-16">
          <nav className="mb-7 flex items-center gap-2 text-sm text-white/40">
            ...
            <span>/</span>
            <span className="text-white/70">For business</span>
          </nav>
          <div className="max-w-2xl">
            <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-[#22d3ee]">For business</p>
            <h1 className="mt-4 font-[family-name:var(--font-fraunces)] text-4xl font-semibold leading-[1.06] tracking-[-0.01em] text-white sm:text-5xl">
              Customers who have already <span className="italic text-[#22d3ee]">done the research</span>
            </h1>
            ...
            <div className="mt-8 flex flex-wrap items-center gap-4">
              <a
                ...
                target="_blank"
                rel="noopener"
                className="group inline-flex items-center gap-2 rounded-full bg-[#0e7490] px-7 py-3.5 text-[15px] font-semibold text-white shadow-[0_10px_30px_-8px_rgba(14,124,102,0.6)] transition-all hover:-translate-y-0.5 hover:bg-[#0891b2]"
              >
                Book a call
              ...
              <a
                ...
                className="text-sm font-semibold text-white underline decoration-white/20 underline-offset-4 hover:decoration-[#22d3ee]"
              >
        ...
        <section className="mx-auto max-w-6xl px-5 py-14 sm:px-8 sm:py-16">
          <div className="grid gap-4 sm:grid-cols-2">
            {offers.map((o) => (
              <div key={o.title} className="flex flex-col rounded-2xl border border-white/[0.08] bg-white/[0.03] p-7 shadow-[0_2px_24px_-16px_rgba(0,0,0,0.2)]">
                <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[#22d3ee]">{o.tag}</p>
                <h2 className="mt-3 font-[family-name:var(--font-fraunces)] text-xl font-semibold tracking-[-0.01em] text-white sm:text-2xl">
                  {o.title}
                </h2>
                <p className="mt-2.5 flex-1 text-sm leading-relaxed text-white/55 sm:text-[15px]">{o.body}</p>
                {o.links && (
                  <ul className="mt-4 space-y-1.5 text-sm">
                    {o.links.map((l) => (
                      <li key={l.href}>
                        <Link href={l.href} className="text-white/70 underline decoration-white/20 underline-offset-4 hover:text-[#22d3ee]">
                          {l.label}
                        </Link>
                      </li>
                    ))}
                ...
                <div className="mt-6">
                  {o.cta.external ? (
                    <a href={o.cta.href} className="group inline-flex items-center gap-1.5 text-sm font-semibold text-[#22d3ee]">
                      {o.cta.label}
                      <ArrowRight className="h-3.5 w-3.5 transition-transform group-hover:translate-x-0.5" />
                    </a>
                  ) : (
                    <Link href={o.cta.href} className="group inline-flex items-center gap-1.5 text-sm font-semibold text-[#22d3ee]">
                      {o.cta.label}
                      <ArrowRight className="h-3.5 w-3.5 transition-transform group-hover:translate-x-0.5" />
                    </Link>
                  )}
          ...
          <p className="mt-10 text-sm">
            <Link href="/blog" className="font-semibold text-[#22d3ee] underline decoration-[#22d3ee]/30 underline-offset-4">
              Read our business insights →
            </Link>{" "}
            <span className="text-white/40">Referral and affiliate strategy for founders and marketers.</span>
          </p>
