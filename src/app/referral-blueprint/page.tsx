...
function SpreadsheetMock({ blurred = false, rows = 6 }: { blurred?: boolean; rows?: number }) {
  ...
  return (
    <div className={`relative rounded-xl border border-[#e5e9e7] overflow-hidden text-[10px] font-mono bg-[#0a1520] ${blurred ? "select-none" : ""}`}>
      {blurred && (
        <>
          <div className="absolute inset-0 backdrop-blur-[2.5px] z-10" style={{ background: "rgba(255,255,255,0.35)" }} />
          <div className="absolute bottom-0 left-0 right-0 h-24 z-20" style={{ background: "linear-gradient(to top, #ffffff 0%, transparent 100%)" }} />
      ...
      {/* Header */}
      <div className="grid bg-[#f5f8f6] border-b border-[#e5e9e7]" style={{ gridTemplateColumns: "120px 80px 90px 80px 50px 1fr" }}>
        {cols.map((c) => (
          <div key={c} className="px-2.5 py-2 text-[9px] font-bold uppercase tracking-wider text-[#9aa39c] truncate">{c}</div>
        ))}
      </div>
      {data.map((row, i) => (
        <div
          key={row.company}
          className="grid border-b border-[#e5e9e7] transition-colors"
          style={{
            gridTemplateColumns: "120px 80px 90px 80px 50px 1fr",
            background: row.highlight ? "rgba(10,167,181,0.07)" : i % 2 === 0 ? "rgba(16,37,27,0.015)" : "transparent",
          }}
        >
          <div className="px-2.5 py-2 text-[#10251b]/85 font-semibold truncate">{row.company}</div>
          ...
          <div className="px-2.5 py-2 font-bold text-[#10251b]/85 truncate">{row.commission}</div>
          <div className="px-2.5 py-2 text-[#6e7b74] truncate">{row.type}</div>
          <div className="px-2.5 py-2 text-[#9aa39c]">{row.cookie}</div>
          <div className="px-2.5 py-2 text-[#6e7b74] truncate">{blurred ? row.angle : row.angle}</div>
        </div>
      ))}
    </div>
  );
...
function DocMock() {
  return (
    <div className="rounded-xl border border-[#e5e9e7] bg-[#0a1520] p-4 text-[10px] font-mono">
      <div className="flex items-center gap-2 mb-3 pb-2 border-b border-[#e5e9e7]">
        <div className="h-2 w-2 rounded-full bg-red-500/70" />
        <div className="h-2 w-2 rounded-full bg-amber-500/70" />
        <div className="h-2 w-2 rounded-full bg-green-500/70" />
        <span className="ml-1 text-[#9aa39c]">strategy-brief.pdf</span>
      </div>
      <div className="space-y-1.5">
        ...
        <div className="flex gap-4 text-[#6e7b74] mb-2">
          <span>Niche: <span className="text-[#10251b]/80 font-semibold">AI Tools</span></span>
          <span>Channel: <span className="text-[#10251b]/80 font-semibold">SEO</span></span>
          <span>Level: <span className="text-[#10251b]/80 font-semibold">Intermediate</span></span>
        </div>
        <div className="h-px bg-[#f5f8f6] my-2" />
        <div className="text-[#3d4b44] font-bold mb-1">WEEK 1-2: Foundation</div>
        {["Register for Jasper, Durable AI, Notion, beehiiv (priority list attached)", "Set up Pretty Links for tracking, installation guide in tool stack PDF", "Build first comparison page: Durable AI vs Wix (full brief in SEO concepts)"].map((t) => (
          <div key={t} className="flex gap-1.5 text-[#9aa39c]">
            <span style={{ color: CYAN }}>✓</span>
            <span>{t}</span>
          </div>
        ))}
        <div className="h-px bg-[#f5f8f6] my-2" />
        <div className="text-[#3d4b44] font-bold mb-1">WEEK 3-4: Content</div>
        ...
        <div className="mt-2 text-[#9aa39c] italic text-[9px]">...continues across 8 pages</div>
      </div>
    </div>
  );
}
...
function SeoMock() {
  ...
  return (
    <div className="rounded-xl border border-[#e5e9e7] bg-[#0a1520] p-4 text-[10px] font-mono space-y-2.5">
      ...
      {keywords.map((k) => (
        <div key={k.kw} className="space-y-1">
          <div className="flex items-center justify-between gap-2">
            <span className="text-[#10251b]/75 text-[9px]">{k.kw}</span>
          ...
          <div className="flex items-center gap-2">
            <div className="flex-1 h-1 rounded-full bg-[#eef1ec] overflow-hidden">
              <div className="h-full rounded-full" style={{ width: `${k.prog}%`, background: CYAN }} />
            </div>
            <span className="text-[#9aa39c] text-[9px] w-8">{k.vol}/mo</span>
          </div>
        </div>
      ))}
    </div>
...
function ChecklistMock() {
  ...
  return (
    <div className="rounded-xl border border-[#e5e9e7] bg-[#0a1520] p-4 text-[10px] font-mono space-y-2">
      ...
      {steps.map((s, i) => (
        <div key={s} className="flex gap-2 items-start">
          <div className="h-4 w-4 rounded border flex items-center justify-center flex-shrink-0 mt-0.5" style={{ borderColor: i < 2 ? CYAN : "rgba(16,37,27,0.12)", background: i < 2 ? `${CYAN}20` : "transparent" }}>
            {i < 2 && <Check className="h-2.5 w-2.5" style={{ color: CYAN }} />}
          </div>
          <span className={i < 2 ? "text-[#9aa39c] line-through" : "text-[#3d4b44]"}>{s}</span>
        </div>
      ))}
    </div>
  );
...
function NichesMock() {
  ...
  return (
    <div className="rounded-xl border border-[#e5e9e7] bg-[#0a1520] p-4 text-[10px] font-mono space-y-2.5">
      ...
      {niches.map((n) => (
        <div key={n.name} className="flex items-center gap-3 rounded-lg p-2" style={{ background: n.rank === 1 ? `${CYAN}10` : "rgba(16,37,27,0.02)", border: `1px solid ${n.rank === 1 ? `${CYAN}30` : "rgba(16,37,27,0.04)"}` }}>
          <div className="h-5 w-5 rounded-full flex items-center justify-center text-[8px] font-black flex-shrink-0" style={{ background: n.rank === 1 ? CYAN : "rgba(16,37,27,0.06)", color: n.rank === 1 ? "#ffffff" : "rgba(16,37,27,0.55)" }}>
            {n.rank}
          </div>
          <div className="flex-1 min-w-0">
            <div className="text-[#10251b]/80 font-semibold">{n.name}</div>
            <div className="text-[#9aa39c]">{n.comm}</div>
          </div>
          <div className="text-right">
            <div className="font-black text-xs" style={{ color: n.rank === 1 ? CYAN : "rgba(16,37,27,0.55)" }}>{n.fit}%</div>
            <div className="text-[#9aa39c] text-[8px]">match</div>
          </div>
        </div>
      ))}
    </div>
  ...
  return (
    <div className="rounded-xl border border-[#e5e9e7] bg-[#0a1520] p-4 text-[10px] font-mono">
      ...
      <div className="grid grid-cols-3 gap-1.5">
        {tools.map((t) => (
          <div key={t.name} className="rounded-lg border border-[#e5e9e7] bg-[#f5f8f6] px-2 py-1.5 text-center">
            <div className="text-[#10251b]/75 font-semibold text-[9px]">{t.name}</div>
            <div className="text-[#9aa39c] text-[8px]">{t.cat}</div>
          </div>
        ))}
      </div>
...
export default function ReferralBlueprintPage() {
  ...
  return (
    ...
    <div className="relative bg-white text-[#10251b]">
      ...
      {/* ── STICKY BOTTOM BAR ─────────────────────────────────────────────── */}
      <div className={`fixed bottom-0 left-0 right-0 z-50 border-t border-[#e5e9e7] bg-white/95 backdrop-blur-md transition-all duration-300 ${stickyVisible ? "translate-y-0 opacity-100" : "translate-y-full opacity-0"}`}>
        <div className="mx-auto max-w-5xl px-5 sm:px-8 py-3.5 flex flex-col sm:flex-row items-center justify-between gap-3">
          <div>
            <p className="text-sm font-black text-[#10251b]">Referral Growth Blueprint</p>
            <p className="text-xs text-[#6e7b74]">250+ programs · Personalised brief · Delivered in 48 hrs · $799 once</p>
          </div>
          <a href="#register" className="flex-shrink-0 inline-flex items-center gap-2 rounded-xl px-6 py-3 text-sm font-black text-white hover:opacity-90 transition-all" style={{ background: AMBER }}>
      ...
      <section ref={heroRef} className="relative pt-12 pb-16 sm:pt-16 sm:pb-20 overflow-hidden border-b border-[#e5e9e7]">
        ...
        <div className="relative mx-auto max-w-6xl px-5 sm:px-8">
          {/* Breadcrumb */}
          <nav className="mb-8 flex items-center gap-2 text-xs text-[#9aa39c]">
            <Link href="/" className="hover:text-[#3d4b44] transition-colors">Refer Labs</Link>
            <span>/</span>
            <Link href="/guides" className="hover:text-[#3d4b44] transition-colors">Guides</Link>
            <span>/</span>
            <span className="text-[#6e7b74]">Referral Growth Blueprint</span>
          </nav>
          ...
          <div className="grid lg:grid-cols-[1fr_520px] gap-10 lg:gap-16 items-start">
            ...
            <div>
              ...
              <h1 className="text-5xl sm:text-6xl lg:text-[58px] font-black tracking-tight leading-[0.96] mb-6 text-[#10251b]">
              ...
              <div className="space-y-2.5 mb-10">
                {[
                  ...
                  <div key={text} className="flex items-start gap-3">
                    ...
                    <span className="text-sm text-[#10251b]/75 leading-relaxed">{text}</span>
                  </div>
                ))}
              </div>

              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 mb-3">
                <a href="#register" className="inline-flex items-center gap-2.5 rounded-xl px-8 py-4 text-base font-black text-white hover:-translate-y-0.5 transition-all shadow-xl" style={{ background: AMBER, boxShadow: `0 12px 40px ${AMBER}40` }}>
                  Get Your Blueprint, $799 AUD
                  <ArrowRight className="h-5 w-5" />
                </a>
                <a href="#whats-inside" className="inline-flex items-center gap-2 rounded-xl px-6 py-4 text-sm font-bold text-[#10251b]/80 hover:text-[#10251b] border border-[#e5e9e7] hover:border-[#e5e9e7] transition-all">
                  See what&apos;s inside first
                </a>
              </div>
              <div className="text-xs text-[#9aa39c] leading-relaxed">
                One-time payment · No subscription · Delivered in 48 hours · Questions? <a href="mailto:jarred@referlabs.com.au" className="underline text-[#3d4b44] hover:text-[#10251b] transition-colors">jarred@referlabs.com.au</a>
              </div>
            </div>
            ...
            <div className="relative">
              ...
              <p className="text-center mt-3 text-[10px] text-[#9aa39c] italic">Actual database preview - blurred for IP protection</p>
            </div>
      ...
      {/* ── STATS BAR ─────────────────────────────────────────────────────── */}
      <section className="border-b border-[#e5e9e7] bg-[#faf9f5]">
        <div className="mx-auto max-w-5xl px-5 sm:px-8 py-6 grid grid-cols-2 sm:grid-cols-4 gap-6 sm:gap-0 sm:divide-x divide-white/10">
          {[
            ...
            <div key={stat} className="text-center sm:px-8">
              <div className="text-2xl font-black text-[#10251b] mb-0.5">{stat}</div>
              <div className="text-xs text-[#9aa39c] leading-relaxed">{label}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="py-20 sm:py-24 border-b border-[#e5e9e7]">
        <div className="mx-auto max-w-5xl px-5 sm:px-8">
          <h2 className="text-3xl sm:text-4xl font-black tracking-tight text-[#10251b] mb-3">
            You want affiliate income but...
          </h2>
          <p className="text-[#6e7b74] mb-12 max-w-xl text-base">
          ...
          <div className="grid sm:grid-cols-3 gap-4 mb-10">
            {[
              ...
              <div key={num} className="rounded-2xl border border-red-500/15 bg-red-500/[0.03] p-6">
                ...
                <h3 className="text-base font-black text-[#10251b] mb-3">{title}</h3>
                <p className="text-sm text-[#6e7b74] leading-relaxed mb-4">{pain}</p>
          ...
          <div className="rounded-2xl border-2 p-6 text-center" style={{ borderColor: `${AMBER}50`, background: `${AMBER}07` }}>
            <p className="text-2xl font-black text-[#10251b] mb-2">There&apos;s a better way.</p>
            <p className="text-sm text-[#3d4b44] max-w-2xl mx-auto">
              The Referral Growth Blueprint is the database (250+ programs, all verified) <strong className="text-[#10251b]">plus</strong> the personalised strategy written for your specific situation. $799 once, delivered in 48 hours. No subscription, no upsell.
            </p>
          </div>
        </div>
      </section>
      ...
      {/* ── NOT A COURSE ──────────────────────────────────────────────────── */}
      <section className="py-16 border-b border-[#e5e9e7] bg-[#f5f8f6]">
        <div className="mx-auto max-w-5xl px-5 sm:px-8">
          <h2 className="text-2xl sm:text-3xl font-black tracking-tight mb-8 text-[#10251b]">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              ...
              <div key={no} className="rounded-2xl border border-[#e5e9e7] bg-[#f5f8f6] p-5">
                <div className="flex items-start gap-2 mb-3">
                  <X className="h-3.5 w-3.5 text-red-400 flex-shrink-0 mt-0.5" />
                  <p className="text-xs text-[#9aa39c] line-through leading-relaxed">{no}</p>
                </div>
                <div className="flex items-start gap-2">
                  <CheckCircle2 className="h-3.5 w-3.5 flex-shrink-0 mt-0.5" style={{ color: CYAN }} />
                  <p className="text-xs font-bold text-[#10251b] leading-relaxed">{yes}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section id="whats-inside" className="py-24 sm:py-32 border-b border-[#e5e9e7] scroll-mt-24">
        <div className="mx-auto max-w-5xl px-5 sm:px-8">
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-14">
            <div>
              <h2 className="text-3xl sm:text-4xl font-black tracking-tight text-[#10251b] mb-2">
                Six deliverables. One payment.
              </h2>
              <p className="text-[#6e7b74] max-w-xl">
                Everything arrives in your inbox within 48 hours. No drip schedule, no course platform, nothing held back.
              </p>
            </div>
            <a href="#register" className="flex-shrink-0 inline-flex items-center gap-2 rounded-xl px-5 py-3 text-sm font-black text-white hover:opacity-90 transition-all" style={{ background: AMBER }}>
              Get All Six - $799
          ...
          <div className="grid lg:grid-cols-2 gap-4">
            {deliverables.map((item, i) => {
              ...
              return (
                <div key={item.num} className={`rounded-2xl border transition-all duration-200 ${isOpen ? "border-[#e5e9e7] bg-[#f5f8f6]" : "border-[#e5e9e7] bg-[#faf9f5] hover:border-[#e5e9e7]"}`}>
                  <button type="button" onClick={() => setOpenDeliverable(isOpen ? null : i)} className="w-full flex items-start gap-4 p-5 text-left">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <span className="text-[9px] font-bold uppercase tracking-widest text-[#9aa39c] block mb-0.5">{item.num}</span>
                          <h3 className="text-sm font-black text-[#10251b]">{item.headline}</h3>
                          <p className="text-xs text-[#6e7b74] mt-0.5">{item.outcome}</p>
                        </div>
                        <div className="h-7 w-7 rounded-lg flex items-center justify-center flex-shrink-0 transition-colors mt-0.5" style={{ background: isOpen ? `${CYAN}25` : "rgba(16,37,27,0.04)" }}>
                          {isOpen ? <ChevronUp className="h-3.5 w-3.5" style={{ color: CYAN }} /> : <ChevronDown className="h-3.5 w-3.5 text-[#9aa39c]" />}
                        </div>
                      </div>
                    </div>
                  </button>
                  ...
                  {isOpen && (
                    <div className="px-5 pb-5 border-t border-[#e5e9e7]">
                      <div className="pt-5 space-y-5">
                        ...
                        <div className="grid sm:grid-cols-3 gap-3">
                          <div className="rounded-xl border border-[#e5e9e7] bg-[#f5f8f6] p-4">
                            ...
                            <p className="text-xs text-[#3d4b44] leading-relaxed">{item.whatItIs}</p>
                          </div>
                          <div className="rounded-xl border border-[#e5e9e7] bg-[#f5f8f6] p-4">
                            <p className="text-[9px] font-bold uppercase tracking-widest mb-2" style={{ color: CYAN }}>Why it matters</p>
                            <p className="text-xs text-[#3d4b44] leading-relaxed">{item.whyItMatters}</p>
                          </div>
                          <div className="rounded-xl border border-[#e5e9e7] bg-[#f5f8f6] p-4">
                            <p className="text-[9px] font-bold uppercase tracking-widest mb-2" style={{ color: CYAN }}>How it&apos;s used</p>
                            <p className="text-xs text-[#3d4b44] leading-relaxed">{item.howUsed}</p>
                          </div>
                        </div>
                        ...
                        <div className="rounded-xl border p-4 flex items-start gap-3" style={{ borderColor: `${AMBER}40`, background: `${AMBER}07` }}>
                          ...
                          <div>
                            ...
                            <p className="text-xs text-[#10251b]/75 font-semibold">{item.valueReplaces}</p>
                          </div>
                        </div>

                        {/* Bullets */}
                        <div>
                          <p className="text-[9px] font-bold uppercase tracking-widest text-[#9aa39c] mb-3">What is included</p>
                          <ul className="grid sm:grid-cols-2 gap-2">
                            {item.bullets.map((b) => (
                              <li key={b} className="flex items-start gap-2">
                                <CheckCircle2 className="h-3.5 w-3.5 flex-shrink-0 mt-0.5" style={{ color: CYAN }} />
                                <span className="text-xs text-[#3d4b44] leading-relaxed">{b}</span>
                              </li>
                            ))}
                          </ul>
                        </div>
                        ...
                        <div className="rounded-xl border border-[#e5e9e7] bg-[#f5f8f6] p-4">
                          <p className="text-[9px] font-bold uppercase tracking-widest text-[#9aa39c] mb-2">What Refer Labs does</p>
                          <p className="text-xs text-[#3d4b44] leading-relaxed">{item.provides}</p>
                        </div>
                      </div>
                    </div>
                  )}
      ...
      <section className="py-24 sm:py-32 border-b border-[#e5e9e7] bg-[#f5f8f6]">
        <div className="mx-auto max-w-5xl px-5 sm:px-8">
          ...
          <div className="grid lg:grid-cols-3 gap-5">
            {/* Sample 1 - Database */}
            <div className="lg:col-span-2 rounded-2xl border border-[#e5e9e7] bg-white p-5">
              <div className="flex items-center gap-2 mb-4">
                <div className="h-7 w-7 rounded-lg flex items-center justify-center" style={{ background: `${CYAN}20`, color: CYAN }}>
                  <FileSpreadsheet className="h-4 w-4" />
                </div>
                <div>
                  <p className="text-xs font-black text-[#10251b]">Affiliate Program Database</p>
                  <p className="text-[10px] text-[#9aa39c]">Sample extract, 10 of 250+ rows shown</p>
                </div>
              </div>
              <SpreadsheetMock rows={10} />
              <p className="text-[10px] text-[#9aa39c] mt-2">* Full database includes 250+ rows across 5 categories with direct program links</p>
            </div>
            ...
            <div className="rounded-2xl border border-[#e5e9e7] bg-white p-5">
              <div className="flex items-center gap-2 mb-4">
                ...
                <div>
                  <p className="text-xs font-black text-[#10251b]">Strategy Brief</p>
                  <p className="text-[10px] text-[#9aa39c]">Sample extract</p>
                </div>
              </div>
              <DocMock />
              <p className="text-[10px] text-[#9aa39c] mt-2">* Full brief is 8+ pages, written for your specific answers</p>
            </div>
            ...
            <div className="rounded-2xl border border-[#e5e9e7] bg-white p-5">
              <div className="flex items-center gap-2 mb-4">
                ...
                <div>
                  <p className="text-xs font-black text-[#10251b]">SEO Page Concepts</p>
                  <p className="text-[10px] text-[#9aa39c]">Sample extract</p>
                </div>
              </div>
              <SeoMock />
              <p className="text-[10px] text-[#9aa39c] mt-2">* 10+ SEO concepts, each with structural page brief</p>
            </div>
            ...
            <div className="rounded-2xl border border-[#e5e9e7] bg-white p-5">
              <div className="flex items-center gap-2 mb-4">
                ...
                <div>
                  <p className="text-xs font-black text-[#10251b]">Niche Selection Brief</p>
                  <p className="text-[10px] text-[#9aa39c]">Sample extract</p>
                </div>
              </div>
              <NichesMock />
              <p className="text-[10px] text-[#9aa39c] mt-2">* Matched to your intake answers, not generic</p>
            </div>
            ...
            <div className="rounded-2xl border border-[#e5e9e7] bg-white p-5">
              <div className="flex items-center gap-2 mb-4">
                ...
                <div>
                  <p className="text-xs font-black text-[#10251b]">Distribution Playbook</p>
                  <p className="text-[10px] text-[#9aa39c]">Sample extract</p>
                </div>
              </div>
              <ChecklistMock />
              <p className="text-[10px] text-[#9aa39c] mt-2">* One playbook per stated channel, action-list format</p>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* ── BEFORE / AFTER ────────────────────────────────────────────────── */}
      <section className="py-24 sm:py-32 border-b border-[#e5e9e7]">
        <div className="mx-auto max-w-5xl px-5 sm:px-8">
          <h2 className="text-3xl sm:text-4xl font-black tracking-tight text-[#10251b] mb-12">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={text} className="flex items-start gap-3">
                    ...
                    <p className="text-sm text-[#6e7b74] leading-relaxed">{text}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* With */}
            <div className="rounded-2xl border border-[#0a7c42]/25 p-6" style={{ background: `${CYAN}07` }}>
              <div className="flex items-center gap-2 mb-6">
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={text} className="flex items-start gap-3">
                    ...
                    <p className="text-sm text-[#10251b]/75 leading-relaxed">{text}</p>
                  </div>
                ))}
              </div>
            </div>
      ...
      <section className="py-24 sm:py-32 border-b border-[#e5e9e7] bg-[#f5f8f6]">
        <div className="mx-auto max-w-5xl px-5 sm:px-8">
          <h2 className="text-3xl sm:text-4xl font-black tracking-tight text-[#10251b] mb-4">
            How we build your blueprint.
          </h2>
          <p className="text-[#6e7b74] max-w-xl mb-14">
          ...
          <div className="grid sm:grid-cols-3 gap-4 items-start">
            ...
            <div className="rounded-2xl border border-[#e5e9e7] bg-[#f5f8f6] p-6">
              <p className="text-[9px] font-bold uppercase tracking-widest text-[#9aa39c] mb-5">You provide</p>
              <div className="space-y-3">
                {[
                  ...
                  <div key={label} className="rounded-xl border border-[#e5e9e7] bg-[#faf9f5] px-4 py-3">
                    <p className="text-xs font-semibold text-[#10251b]/80">{label}</p>
                    <p className="text-[10px] text-[#9aa39c] mt-0.5">{example}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="flex flex-col items-center justify-center gap-6 py-6">
              ...
              <div className="text-center">
                ...
                <p className="text-sm font-black text-[#10251b]">Delivered in 48 hrs</p>
                <p className="text-xs text-[#9aa39c] mt-1 max-w-[160px] mx-auto">Six files to your inbox. Everything at once.</p>
              </div>
            </div>

            {/* Outputs */}
            <div className="rounded-2xl border border-[#e5e9e7] bg-[#f5f8f6] p-6">
              <p className="text-[9px] font-bold uppercase tracking-widest text-[#9aa39c] mb-5">You receive</p>
              <div className="space-y-3">
                {[
                  ...
                  <div key={label} className="flex items-start gap-2.5 rounded-xl border border-[#e5e9e7] bg-[#faf9f5] px-3 py-2.5">
                    <CheckCircle2 className="h-3.5 w-3.5 flex-shrink-0 mt-0.5" style={{ color: CYAN }} />
                    <div>
                      <p className="text-xs font-semibold text-[#10251b]/80">{label}</p>
                      <p className="text-[10px] text-[#9aa39c] mt-0.5">{note}</p>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <section className="py-24 sm:py-32 border-b border-[#e5e9e7]">
        <div className="mx-auto max-w-5xl px-5 sm:px-8">
          <h2 className="text-3xl font-black tracking-tight text-[#10251b] mb-14">From order to execution in 48 hours.</h2>
          <div className="grid sm:grid-cols-4 gap-0 relative">
            ...
            {[
              ...
              <div key={label} className="flex flex-col items-center text-center relative px-4">
                <div className="h-10 w-10 rounded-full border-2 flex items-center justify-center mb-4 z-10 bg-white" style={{ borderColor: color, background: done ? `${color}20` : "#f5f8f6" }}>
                ...
                <p className="text-sm font-black text-[#10251b] mb-1.5">{label}</p>
                <p className="text-xs text-[#9aa39c] leading-relaxed">{desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-24 sm:py-32 border-b border-[#e5e9e7] bg-[#f5f8f6]">
        <div className="mx-auto max-w-5xl px-5 sm:px-8">
          <h2 className="text-3xl sm:text-4xl font-black tracking-tight mb-3 text-[#10251b]">
            Built for people who want results, not homework.
          </h2>
          <p className="text-[#6e7b74] text-lg mb-12 max-w-xl">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {[
              ...
              <div key={label} className="flex items-start gap-3.5 rounded-xl border border-[#e5e9e7] bg-[#f5f8f6] p-5">
                ...
                <div>
                  <p className="text-sm font-bold text-[#10251b] mb-0.5">{label}</p>
                  <p className="text-xs text-[#6e7b74] leading-relaxed">{desc}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="py-24 sm:py-32 border-b border-[#e5e9e7]">
        <div className="mx-auto max-w-5xl px-5 sm:px-8">
          <h2 className="text-3xl font-black tracking-tight text-[#10251b] mb-12">
          ...
          <div className="grid lg:grid-cols-2 gap-4 max-w-4xl">
            {[
              ...
              <div key={q} className="rounded-2xl border border-[#e5e9e7] bg-[#f5f8f6] p-6">
                <div className="flex items-start gap-3 mb-3">
                  <AlertCircle className="h-4 w-4 flex-shrink-0 mt-0.5" style={{ color: AMBER }} />
                  <p className="text-sm font-black text-[#10251b]">{q}</p>
                </div>
                <p className="text-sm text-[#3d4b44] leading-relaxed pl-7">{a}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mx-auto max-w-5xl px-5 sm:px-8">
          <div className="grid lg:grid-cols-[280px_1fr] gap-12 lg:gap-16 items-start">
            <div className="rounded-2xl border border-[#e5e9e7] bg-[#f5f8f6] p-6">
              ...
              <a href="mailto:jarred@referlabs.com.au" className="mt-5 flex items-center gap-2 text-xs text-[#9aa39c] hover:text-[#3d4b44] transition-colors">
            ...
            <div>
              ...
              <div className="mt-8 grid sm:grid-cols-2 gap-3">
                {[
                  ...
                  <div key={no} className="rounded-xl border border-[#e5e9e7] bg-[#faf9f5] p-4">
                    <div className="flex items-start gap-2 mb-2.5">
                      <X className="h-3.5 w-3.5 text-red-400 flex-shrink-0 mt-0.5" />
                      <p className="text-xs text-[#9aa39c] line-through">{no}</p>
                    </div>
                    <div className="flex items-start gap-2">
                      <CheckCircle2 className="h-3.5 w-3.5 flex-shrink-0 mt-0.5" style={{ color: CYAN }} />
                      <p className="text-xs font-semibold text-[#10251b]/80">{yes}</p>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <section className="py-24 sm:py-32 border-b border-[#e5e9e7] bg-[#f5f8f6]">
        <div className="mx-auto max-w-5xl px-5 sm:px-8">
          <h2 className="text-3xl font-black tracking-tight mb-14 text-[#10251b]">From people who have used it.</h2>
          <div className="grid sm:grid-cols-3 gap-5">
            {[
              ...
              <div key={t.name} className="rounded-2xl border border-[#e5e9e7] bg-[#f5f8f6] p-7 flex flex-col gap-5">
                <div className="flex gap-0.5">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <Star key={i} className="h-3 w-3 fill-current" style={{ color: AMBER }} />
                  ))}
                </div>
                <p className="text-sm text-[#10251b]/75 leading-relaxed flex-1">&ldquo;{t.text}&rdquo;</p>
                <div className="border-t border-[#e5e9e7] pt-4">
                  <p className="text-sm font-bold text-[#10251b]">{t.name}</p>
                  <p className="text-xs text-[#9aa39c]">{t.role}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="mx-auto max-w-5xl px-5 sm:px-8">
          <div className="flex flex-col lg:flex-row gap-14 lg:gap-20 items-start">
            ...
            <div className="lg:flex-1">
              <h2 className="text-4xl sm:text-5xl font-black tracking-tight mb-6 leading-tight text-[#10251b]">
                The cost of not buying this.
              </h2>
              <div className="space-y-4 text-[#3d4b44] leading-relaxed max-w-lg text-base mb-8">
                ...
                <p className="text-[#10251b] font-semibold">$799 buys you the starting point most people never reach.</p>
              </div>

              <div className="rounded-2xl border border-[#e5e9e7] bg-[#f5f8f6] p-6 max-w-sm">
                ...
                <div className="space-y-3">
                  {[
                    ...
                    <div key={item} className="flex items-center justify-between gap-4 text-sm">
                      <span className="text-[#6e7b74]">{item}</span>
                      <span className="text-red-400 font-semibold flex-shrink-0">{value}</span>
                    </div>
                  ))}
                  <div className="border-t border-[#e5e9e7] pt-3 flex items-center justify-between gap-4 text-sm">
                    <span className="text-[#10251b] font-bold">Referral Growth Blueprint</span>
                    <span className="font-black text-[#10251b]">$799</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="lg:w-80 flex-shrink-0 w-full">
              <div className="rounded-2xl border-2 p-8" style={{ borderColor: `${AMBER}60`, background: `${AMBER}06` }}>
                <p className="text-[10px] font-bold uppercase tracking-[0.2em] mb-1 text-[#6e7b74]">Referral Growth Blueprint</p>
                <div className="flex items-baseline gap-2 mb-6">
                  <span className="text-4xl font-black text-[#10251b]">$799</span>
                  <span className="text-[#9aa39c] text-sm">AUD - one-time</span>
                </div>
                <ul className="space-y-3 mb-8">
                  {[
                    ...
                    <li key={item} className="flex items-start gap-2.5">
                      ...
                      <span className="text-sm text-[#3d4b44]">{item}</span>
                    </li>
                  ))}
                </ul>
                <a href="#register" className="flex items-center justify-center gap-2 w-full rounded-xl py-4 text-sm font-black text-white hover:opacity-90 transition-all" style={{ background: AMBER }}>
                  Get the Full System in 48 Hours
                  <ArrowRight className="h-4 w-4" />
                </a>
                <div className="flex items-center justify-center gap-1.5 mt-4">
                  <Lock className="h-3 w-3 text-[#9aa39c]" />
                  <p className="text-xs text-[#9aa39c]">Secure checkout via Stripe</p>
                </div>
              </div>
              <div className="mt-4 rounded-xl border border-[#e5e9e7] px-4 py-3 flex items-start gap-2.5">
                <Lock className="h-3.5 w-3.5 text-[#9aa39c] flex-shrink-0 mt-0.5" />
                <p className="text-xs text-[#9aa39c]">
                  Questions before buying? Email{" "}
                  <a href="mailto:jarred@referlabs.com.au" className="underline text-[#3d4b44] hover:text-[#10251b] transition-colors">jarred@referlabs.com.au</a>
                  {" "}- same day response.
                </p>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="flex-1 max-w-xl space-y-8">
              <div className="grid sm:grid-cols-2 gap-6">
                <div>
                  ...
                  <input type="text" value={form.name} onChange={(e) => set("name", e.target.value)} placeholder="Your name" required className="w-full border-b-2 border-black/15 pb-3 text-sm text-black placeholder:text-black/30 focus:outline-none focus:border-[#0a7c42] transition-colors bg-transparent" />
                </div>
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-widest text-black/45 mb-2">
                    Email <span style={{ color: CYAN }}>*</span>
                  </label>
                  <input type="email" value={form.email} onChange={(e) => set("email", e.target.value)} placeholder="you@example.com" required className="w-full border-b-2 border-black/15 pb-3 text-sm text-black placeholder:text-black/30 focus:outline-none focus:border-[#0a7c42] transition-colors bg-transparent" />
                </div>
              </div>
              ...
              <div>
                ...
                <input type="url" value={form.website} onChange={(e) => set("website", e.target.value)} placeholder="https://yoursite.com" className="w-full border-b-2 border-black/15 pb-3 text-sm text-black placeholder:text-black/30 focus:outline-none focus:border-[#0a7c42] transition-colors bg-transparent" />
              </div>
              ...
              <div>
                ...
                <input type="text" value={form.industry} onChange={(e) => set("industry", e.target.value)} placeholder="e.g. AI tools, health, SaaS, fintech" className="w-full border-b-2 border-black/15 pb-3 text-sm text-black placeholder:text-black/30 focus:outline-none focus:border-[#0a7c42] transition-colors bg-transparent" />
              </div>
                ...
                <div className="grid sm:grid-cols-2 gap-2">
                  {primaryGoalOptions.map((opt) => (
                    <button key={opt} type="button" onClick={() => set("primaryGoal", opt)}
                      className={`text-left px-4 py-3 rounded-xl border text-sm transition-all ${form.primaryGoal === opt ? "border-[#0a7c42] bg-[#0a7c42]/[0.07] text-black font-semibold" : "border-black/12 text-black/50 hover:border-black/25 hover:text-black/75"}`}>
                      {opt}
                    </button>
                  ))}
                </div>
                  ...
                  {channelOptions.map((ch) => {
                    ...
                    return (
                      <button key={ch} type="button" onClick={() => toggleChannel(ch)}
                        className={`flex items-center gap-3 text-left px-4 py-3 rounded-xl border text-sm transition-all ${active ? "border-[#0a7c42] bg-[#0a7c42]/[0.07] text-black font-semibold" : "border-black/12 text-black/50 hover:border-black/25 hover:text-black/75"}`}>
                        <div className={`h-4 w-4 rounded border-2 flex items-center justify-center flex-shrink-0 transition-all ${active ? "border-[#0a7c42] bg-[#0a7c42]" : "border-black/25"}`}>
                          {active && <Check className="h-3 w-3 text-[#10251b]" />}
                        </div>
                        {ch}
                      </button>
                    );
                ...
                <div className="grid grid-cols-3 gap-2">
                  {["Beginner", "Intermediate", "Advanced"].map((lvl) => (
                    <button key={lvl} type="button" onClick={() => set("experienceLevel", lvl)}
                      className={`py-3 rounded-xl border text-sm font-semibold transition-all ${form.experienceLevel === lvl ? "border-[#0a7c42] bg-[#0a7c42]/[0.07] text-black" : "border-black/12 text-black/45 hover:border-black/25"}`}>
                      {lvl}
                    </button>
                  ))}
                </div>
              ...
              <button type="submit" disabled={submitting}
                className="flex items-center justify-center gap-3 w-full rounded-xl py-4 text-base font-black text-white disabled:opacity-50 disabled:cursor-not-allowed hover:opacity-90 transition-all"
      ...
      {/* ── FAQ ───────────────────────────────────────────────────────────── */}
      <section className="py-24 sm:py-32 border-t border-[#e5e9e7]">
        <div className="mx-auto max-w-5xl px-5 sm:px-8">
          <h2 className="text-3xl font-black tracking-tight mb-12 text-[#10251b]">Common questions</h2>
          <div className="max-w-2xl space-y-1">
            {faqs.map((item, i) => (
              <div key={item.q} className="border-b border-[#e5e9e7]">
                <button onClick={() => setOpenFaq(openFaq === i ? null : i)}
                  className="flex items-center justify-between w-full py-5 text-left gap-4 group">
                  <h3 className="text-sm font-bold text-[#10251b] group-hover:text-[#0a7c42] transition-colors">{item.q}</h3>
                  {openFaq === i
                    ? <ChevronUp className="h-4 w-4 flex-shrink-0 text-[#9aa39c]" />
                    : <ChevronDown className="h-4 w-4 flex-shrink-0 text-[#9aa39c]" />
                  }
                </button>
                {openFaq === i && (
                  <p className="pb-5 text-sm text-[#3d4b44] leading-relaxed">{item.a}</p>
                )}
              </div>
            ))}
          </div>
      ...
      {/* ── SEO CONTENT ───────────────────────────────────────────────────── */}
      <section className="py-16 border-t border-[#e5e9e7]">
        <div className="mx-auto max-w-5xl px-5 sm:px-8">
          <div className="grid lg:grid-cols-3 gap-10">
            <div>
              <h2 className="text-base font-black text-[#10251b] mb-3">A referral marketing blueprint built in-house</h2>
              <p className="text-sm text-[#6e7b74] leading-relaxed">
            ...
            <div>
              <h2 className="text-base font-black text-[#10251b] mb-3">How to start affiliate marketing in Australia</h2>
              <p className="text-sm text-[#6e7b74] leading-relaxed">
            ...
            <div>
              <h2 className="text-base font-black text-[#10251b] mb-3">Best affiliate programs Australia 2026</h2>
              <p className="text-sm text-[#6e7b74] leading-relaxed">
                The database covers 250+ programs including AI tools (20-40% recurring commissions), SaaS platforms (15-50% recurring), health and telehealth AU ($50-$150 CPA), startup tools (25-50% one-time), and fintech ($30-$200 per referral). Full commission data, cookie windows, and program links included for every entry. <Link href="/guides" className="underline text-[#3d4b44] hover:text-[#10251b] transition-colors">Browse individual program reviews</Link> on Refer Labs.
              </p>
            </div>
          </div>
        </div>
      ...
      {/* ── RELATED GUIDES ────────────────────────────────────────────────── */}
      <section className="py-20 border-t border-[#e5e9e7] bg-[#f5f8f6]">
        <div className="mx-auto max-w-5xl px-5 sm:px-8">
          <h2 className="text-2xl font-black tracking-tight mb-2 text-[#10251b]">Preview programs from the database</h2>
          <p className="text-[#6e7b74] text-sm mb-8 max-w-lg">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              ...
              <Link key={p.href} href={p.href}
                className="group rounded-xl border border-[#e5e9e7] bg-[#faf9f5] p-4 hover:border-[#0a7c42]/30 hover:bg-[#0a7c42]/[0.04] transition-all">
                <p className="text-[9px] font-bold uppercase tracking-widest mb-2" style={{ color: CYAN }}>{p.cat}</p>
                <p className="text-sm font-bold text-[#10251b] mb-1 group-hover:text-[#0a7c42] transition-colors">{p.label}</p>
                <p className="text-xs text-[#9aa39c] mb-3">{p.comm}</p>
                <p className="text-xs font-semibold transition-colors" style={{ color: CYAN }}>View review →</p>
              </Link>
            ))}
          </div>
          <p className="text-xs text-[#9aa39c] mt-4">
            The full database includes 250+ programs.{" "}
            <Link href="/guides" className="underline text-[#9aa39c] hover:text-[#10251b] transition-colors">Browse all guides</Link> on Refer Labs.
          </p>
        </div>
      </section>

      {/* ── FINAL CTA ─────────────────────────────────────────────────────── */}
      <section className="border-t border-[#e5e9e7] bg-white py-20 sm:py-24">
        <div className="mx-auto max-w-5xl px-5 sm:px-8 text-center">
          <h2 className="text-3xl sm:text-4xl font-black tracking-tight mb-4 text-[#10251b]">
            Get the full system in 48 hours.
          </h2>
          <p className="text-[#6e7b74] text-base max-w-md mx-auto mb-8">
          ...
          <div className="flex items-center justify-center gap-3 mb-10 flex-wrap">
            {[
              ...
              <div key={label} className="flex items-center gap-1.5 rounded-full px-3 py-1.5 border border-[#e5e9e7] bg-[#f5f8f6]">
                <span style={{ color: CYAN }}>{icon}</span>
                <span className="text-xs text-[#3d4b44]">{label}</span>
              </div>
            ))}
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <a href="#register"
              className="inline-flex items-center gap-2 rounded-xl px-10 py-4 text-base font-black text-white hover:-translate-y-0.5 transition-all shadow-xl"
              style={{ background: AMBER, boxShadow: `0 12px 40px ${AMBER}35` }}>
              Get the Blueprint - $799 AUD
              <ArrowRight className="h-5 w-5" />
            </a>
            <Link href="/contact" className="text-sm text-[#9aa39c] hover:text-[#10251b] transition-colors">
              Questions first? Contact us
            </Link>
          </div>

          <p className="text-xs text-[#9aa39c] mt-6">One-time payment - No subscription - Delivered within 48 hours of purchase</p>
        </div>
      </section>
