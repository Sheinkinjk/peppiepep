    ...
    <ConsumerShell>
      ...
      <main id="main-content" className="relative mx-auto max-w-3xl px-5 sm:px-8 lg:px-12 pb-24 pt-12 sm:pt-16">
        <nav className="mb-8 flex flex-wrap items-center gap-2 text-sm text-white/55">
          ...
          <span>/</span>
          <span className="text-white/70">Carrd vs Durable AI</span>
        </nav>

        <p className="text-white/40 text-xs mb-5">Updated June 2026</p>
        ...
        <p className="text-white/55 text-base sm:text-lg leading-relaxed mb-6 max-w-2xl">
          Both are excellent — for different jobs. <strong className="text-white/70">Carrd</strong> is the cheapest way to
          ship a clean one-page site. <strong className="text-white/70">Durable AI</strong> generates a full business
        ...
        <div className="rounded-xl border px-6 py-5 mb-10" style={{ borderColor: `${CYAN}40`, background: `${CYAN}0A` }}>
          ...
          <p className="text-white/70 text-sm sm:text-base leading-relaxed mb-5">
            Want one clean page for the least money? <strong className="text-white">Carrd.</strong> Want a full,
            multi-page business site generated by AI — with a CRM and invoicing built in? <strong className="text-white">Durable AI.</strong>
          </p>
          <div className="flex flex-wrap gap-3">
            ...
            <a {...aff(DURABLE_URL)} data-cta="verdict-durable" className="inline-flex items-center gap-2 rounded-xl border px-5 py-2.5 text-sm font-semibold text-white/70 transition-all hover:text-white" style={{ borderColor: `${CYAN}40` }}>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-white/[0.03]">
                ...
                <th className="text-left font-black text-white px-4 py-3">Carrd</th>
                <th className="text-left font-black text-white px-4 py-3">Durable AI</th>
              </tr>
            </thead>
            <tbody>
              {rows.map((r) => (
                <tr key={r.label} className="border-t border-white/[0.08] align-top">
                  <td className="px-4 py-3 text-white/55 font-medium">{r.label}</td>
                  <td className="px-4 py-3 text-white/70">{r.carrd}</td>
                  <td className="px-4 py-3 text-white/70">{r.durable}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="grid sm:grid-cols-2 gap-4 mb-12">
          <div className="rounded-xl border border-white/[0.08] bg-white/[0.03] p-6">
            ...
            <p className="text-white/55 text-sm leading-relaxed mb-4">Cheapest one-page builder. Free to start, Pro from ~$9/year.</p>
            ...
            <p className="mt-3"><Link href="/carrd" className="text-xs text-white/55 underline decoration-white/20 underline-offset-2 hover:text-white/70">Read our Carrd review →</Link></p>
          </div>
          <div className="rounded-xl border border-white/[0.08] bg-white/[0.03] p-6">
            <h3 className="text-lg font-bold mb-2">Go with Durable AI</h3>
            <p className="text-white/55 text-sm leading-relaxed mb-4">Full business site in ~30s, with CRM and invoicing.</p>
            ...
            <p className="mt-3"><Link href="/durableai" className="text-xs text-white/55 underline decoration-white/20 underline-offset-2 hover:text-white/70">Read our Durable AI review →</Link></p>
          </div>
        </div>
